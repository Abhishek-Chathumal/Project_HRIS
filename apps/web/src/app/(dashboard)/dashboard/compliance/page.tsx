...
const log = createLogger('CompliancePage');
...
const complianceChecks: ComplianceCheck[] = [
  {
    ...
    description: 'All PII fields encrypted with AES-256-GCM',
    status: 'compliant',
  ...
  },
  {
    ...
    description: 'Users can export all personal data in machine-readable format',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'high',
  },
  {
    ...
    description: 'Personal data anonymization and deletion on request',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'high',
  },
  {
    ...
    description: 'Structured JSON export of user data',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'medium',
  },
  {
    ...
    description: 'Explicit consent tracking for data processing activities',
    status: 'partial',
  ...
  },
  {
    ...
    description: 'DPA with all third-party processors',
    status: 'partial',
  ...
  },
  {
    ...
    category: 'soc2',
    name: 'Access Control',
    description: 'RBAC with least-privilege principle enforced',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'high',
  },
  {
    ...
    category: 'soc2',
    name: 'Audit Logging',
    description: 'All CRUD operations logged with user, IP, and timestamp',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'high',
  },
  {
    ...
    description: 'Git Flow with PR reviews and conventional commits',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'medium',
  },
  {
    ...
    description: 'Documented incident response with escalation procedures',
    status: 'partial',
  ...
  },
  {
    ...
    category: 'ccpa',
    name: 'Do Not Sell',
    description: 'No personal data sold to third parties',
    status: 'compliant',
    lastAudit: '2026-02-20',
    criticality: 'high',
  },
...
const retentionPolicies: RetentionPolicy[] = [
  {
    ...
    recordCount: 12450,
    nextEnforcement: '2026-03-01',
  },
  {
    resource: 'Sessions',
    retentionDays: 90,
    action: 'delete',
    description: 'Expired sessions cleaned after 90 days',
    recordCount: 340,
    nextEnforcement: '2026-02-25',
  },
  {
    resource: 'Job Applications',
    retentionDays: 730,
    action: 'anonymize',
    description: 'Rejected applicant data anonymized after 2 years',
    recordCount: 890,
    nextEnforcement: '2026-03-01',
  },
  {
    ...
    retentionDays: 1825,
    action: 'archive',
    description: 'Archived after 5 years',
    recordCount: 45200,
    nextEnforcement: '2026-06-01',
  },
  {
    resource: 'Payroll Records',
    retentionDays: 2555,
    action: 'archive',
    description: '7-year retention for tax compliance',
    recordCount: 8400,
    nextEnforcement: '2026-06-01',
  },
  {
    ...
    retentionDays: 1095,
    action: 'anonymize',
    description: 'Anonymized after 3 years',
    recordCount: 1200,
    nextEnforcement: '2026-04-01',
  },
];
...
export default function CompliancePage() {
  ...
  const filtered = categoryFilter
    ? complianceChecks.filter((c) => c.category === categoryFilter)
    : complianceChecks;
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
          <button
            ...
            onClick={() => {
              log.info('ExportAudit', 'Exporting compliance audit report');
              toast.success('Export Started', 'Compliance audit report is being generated.');
            }}
          >
            <svg
              width="16"
          ...
          <button
            ...
            onClick={() => {
              log.info('RunScan', 'Running compliance scan');
              toast.info(
                'Compliance Scan',
                'Scanning all compliance checks across GDPR, CCPA, SOC 2...',
              );
              setTimeout(
                () =>
                  toast.success(
                    'Scan Complete',
                    `Compliance score: ${Math.round((complianceChecks.filter((c) => c.status === 'compliant').length / complianceChecks.length) * 100)}%`,
                  ),
                2000,
              );
            }}
        ...
        <>
          <div style={{ display: 'flex', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
            ...
            {Object.entries(categoryLabels).map(([key, cfg]) => (
              <button
                key={key}
                className={`btn btn-sm ${categoryFilter === key ? 'btn-primary' : 'btn-secondary'}`}
                    ...
                    <tr key={check.id}>
                      ...
                      <td>
                        <span className="badge badge-neutral">
                          {categoryLabels[check.category]?.icon}{' '}
                      ...
                      <td>
                        <span
                          className={`badge ${check.criticality === 'high' ? 'badge-danger' : check.criticality === 'medium' ? 'badge-warning' : 'badge-info'}`}
                        >
                          {check.criticality}
                        </span>
                      </td>
                      <td className="text-xs text-secondary">
                        {new Date(check.lastAudit).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                        })}
                      </td>
                      <td>
                        <span className={`badge ${statusConfig[check.status]?.class}`}>
        ...
        <div className="card">
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                log.info('EnforceRetention', 'Enforcing data retention policies');
                toast.warning(
                  'Enforce Retention',
                  'Running data retention enforcement across all resources...',
                );
              }}
            >
              Enforce Now
                ...
                {retentionPolicies.map((policy, i) => (
                  <tr key={i}>
                    ...
                    <td className="font-mono text-sm">
                      {Math.round(policy.retentionDays / 365)} years
                    </td>
                    <td>
                      <span className={`badge ${actionConfig[policy.action]?.class}`}>
                        {actionConfig[policy.action]?.label}
                      </span>
                    </td>
                    ...
                    <td className="text-xs text-secondary">
                      {new Date(policy.nextEnforcement).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </td>
              ...
              {[
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: item.status ? 'var(--success)' : 'var(--danger)' }}
                  >
                    {item.value}
                  </span>
                </div>
