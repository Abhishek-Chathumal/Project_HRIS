...
export class LdapService implements OnModuleDestroy {
    ...
    private loadConfig(): LdapConfig {
        return {
            ...
            searchFilter: this.configService.get<string>('LDAP_SEARCH_FILTER', '(sAMAccountName={{username}})'),
            ...
            groupSearchFilter: this.configService.get<string>('LDAP_GROUP_SEARCH_FILTER', '(member={{dn}})'),
            ...
            roleMapping: this.parseRoleMapping(
                this.configService.get<string>('LDAP_ROLE_MAPPING', '{}'),
            ),
        };
    }
    ...
    async authenticate(username: string, password: string): Promise<LdapUser | null> {
        ...
        try {
            ...
            const filter = this.config.searchFilter.replace('{{username}}', ldap.parseDN(username).toString() || username);
    ...
    private async getUserGroups(client: ldap.Client, userDN: string): Promise<string[]> {
        try {
            ...
            return entries.map((entry) => {
                const attr = entry.attributes?.[this.config.groupMemberAttribute];
    ...
    async syncDirectoryUsers(): Promise<LdapUser[]> {
        ...
        try {
            ...
            const entries = await this.searchAll(client, this.config.searchBase, {
                filter: '(&(objectClass=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))',
                scope: 'sub',
                attributes: [
                    this.config.usernameAttribute,
                    this.config.emailAttribute,
