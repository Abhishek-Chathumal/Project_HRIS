...
export class AttendanceController {
  ...
  async getMyStatus(@CurrentUser() user: { id: string }) {
    ...
    if (!employee) return { isClockedIn: false, clockInTime: null, clockOutTime: null };
    return this.attendanceService.getTodayStatus(employee.id);
  }

  @Get('today')
  @ApiOperation({ summary: 'Get all employees attendance for today (admin view)' })
  async getTodayAttendance() {
    return this.attendanceService.getTodayAll();
  }
