...
export class AttendanceService {
  ...
  async clockIn(
    employeeId: string,
    data: { source?: string; ipAddress?: string; geoLocation?: object; locationId?: string },
  ) {
  ...
  async clockOut(employeeId: string) {
    ...
    const workMinutes = Math.floor((clockOut.getTime() - record.clockIn!.getTime()) / 60000);
  ...
  async getAttendanceSummary(employeeId: string, month: number, year: number) {
    ...
    if (summary.present > 0) {
      summary.averageWorkHours =
        Math.round((summary.totalWorkMinutes / summary.present / 60) * 10) / 10;
    }
  ...
  async getTodayAll() {
    ...
    // Get total active employees for calculating absent count
    ...
    const absent = totalActive - present - halfDay - onLeave;
